...
func loadUsers(usersFilename []string) (Users, error) {
	...
	for _, f := range usersFilename {
		...
		if err != nil {
			return nil, errors.WithMessagef(err, "problème avec la fiche utilisateur %s", f)
		}
		if err = yaml.Unmarshal(data, &current); err != nil {
			return nil, errors.WithMessagef(err, "problème avec le contenu de la fiche utilisateur %s", f)
		}
		users.addUser(current)
	}
	if len(users) <= 0 {
		return nil, errors.New("aucune fiche utilisateur chargée")
	}
	return users, nil
}
